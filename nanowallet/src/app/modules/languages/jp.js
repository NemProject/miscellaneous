...
　　　　　　　　　　　　APOSTILLE_REQUEST_MESSAGE: 'リクエストメッセージ',
      ...
      //SIGNUP_COMMON_WALLET_WARNING: 'ウォレットは、<a><b>一時的に</b></a>ブラウザ内のローカルストレージに保存されます。ウォレット作成後に自動的にダウンロードされる.wltファイルはバックアップとして利用します。ブラウザのローカルストレージが削除された場合は、このファイルからインポート可能です。バックアップとして、.wltファイルを保存していない場合は、アカウントにログインして、秘密鍵をコピーし安全な場所にバックアップして下さい。バックアップされている事を確認してから、自己の責任に於いてアカウントに資金を送金してください。',
      //SIGNUP_COMMON_WALLET_WARNING_FOOTER: '以下をクリックすると、上記の警告を読み、理解したことに同意したことになります。',
      SIGNUP_COMMON_WALLET_WARNING_TITLE: 'アカウントを安全に仕様する為に!',
      SIGNUP_COMMON_WALLET_WARNING_1: 'ウォレットはブラウザのローカルストレージに<b><u>一時的に</u></b>保存されます。ブラウザは、任意のアドオン等によりローカルストレージをクリアしてしまう事があります。 これはデータの損失につながり、簡単に取り消すことはできません。 あなたのウォレットが消去される為、あなたのアカウントを復元するために必要な情報がすべて揃っていることを、確認することが非常に重要です。',
      ...
      SIGNUP_COMMON_WALLET_WARNING_5: 'あなたの秘密鍵がバックアップされていることを確認した後に、あなたのアカウントに資金を送金するのもあなたの責任です。',
      SIGNUP_COMMON_WALLET_WARNING_BTN_1: '生のウォレットファイルの表示',
      SIGNUP_COMMON_WALLET_WARNING_BTN_1_INFO: 'ウォレットファイルを作成するには、空のテキスト・ファイルを作成し、上記のBase64で作成されたキーを記入します。 <br>ファイルを<b><i> yourWalletName.wlt</i></b>という名前で保存して、インポートすることができます。',
