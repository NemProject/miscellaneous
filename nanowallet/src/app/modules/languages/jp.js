function JapaneseProvider($translateProvider) {
    ...
    $translateProvider.translations('jp', {
      ...
      DASHBOARD_NOTICE_3: 'NEM チームは、メインネット上でこのアプリケーションを利用することに起因する、資金の消失について責任を負いません。初めにテストネットにて NanoWallet の操作を行い、利用方法を習得する事をお勧めします。利用者は自身の資金を管理し、秘密鍵のバックアップを行う必要があります。',
      DASHBOARD_NOTICE_4: 'すべての資金が安全にバックアップされていると確信しない限り、メインネットを使用しないでください。紙に秘密鍵を書き留めて、安全な場所に保管することをお勧めします。さらに、アカウントを Lightwallet / NCC にインポートもできます。',
      ...
      ACCOUNT_PRIVATE_KEY_NOTE: "秘密鍵は最重要機密です。.wlt ファイル、紙、写真、<b><u>エクスポートウォレット QR</u></b>など、<b><u>オフライン</u></b>で安全に保管してしてください。\n",
      ...
      ACCOUNT_ADD_NEW_ACCOUNT_WARNING: "BIP32 方式を使用しているのですべてのアカウントのアドレスはマスター秘密鍵から生成されています。秘密鍵とパスワードは全てのアカウントを復帰させるために絶対に必要です。ローカルストレージが削除された場合は操作を繰り返さないために新規アカウントを追加した後にあなたのウォレットをバックアップする事を強くお勧めします。",
      ...
      SIGNUP_COMMON_WALLET_WARNING_1: 'ウォレットはブラウザのローカルストレージに<b><u>一時的に</u></b>保存されます。ブラウザは、任意のアドオン等によりローカルストレージがクリアされてしまう事があります。 これはデータの損失につながり、簡単に取り消すことはできません。 あなたのウォレットが消去された時の為に、あなたのアカウントを復元する為の必要な情報がすべて揃っていることを確認することが非常に重要です。',
      ...
      SIGNUP_COMMON_WALLET_WARNING_CONFIRM_2: '秘密鍵をバックアップしました',
